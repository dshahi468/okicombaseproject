...
class AuthController {
  ...
  private initRoutes() {
    this.router.post('/register', this.upload.none(), this.validateBody('signup'), this.signup)
    this.router.post('/login', this.upload.none(), this.validateBody('signin'), this.signin)
    this.router.post('/verify-email', this.upload.none(), this.validateBody('verify'), this.verify)
    this.router.post(
  ...
  async signup(req: Request, res: Response) {
    ...
    await cognito
      ...
      .then((response) => {
        return res
          .status(200)
          .json({ message: 'Sign up successful', result: JSON.stringify(response) })
      })
      .catch((error) => {
  ...
  async verify(req: Request, res: Response) {
    ...
    await cognito
      ...
      .then((response) => {
        return res
          .status(200)
          .json({ message: 'Email verified successfully.', result: JSON.stringify(response) })
      })
      .catch((error) => {
        return res.status(500).json({ error: error })
      })
  ...
  async confirmpassword(req: Request, res: Response) {
    ...
    await cognito
      ...
      .then((response) => {
        return res
          .status(200)
          .json({ message: 'Password reset successfully.', result: JSON.stringify(response) })
      })
      .catch((error) => {
        return res.status(500).json({ error: error })
      })
